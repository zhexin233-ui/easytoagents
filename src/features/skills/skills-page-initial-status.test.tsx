...
describe("全局 Skills 检测与复制导入", () => {
  it("接管候选与复制候选分组独立选择，提交后直接消费接管预览", async () => {
    ...
    expect(
      screen.queryByRole("dialog", { name: /确认/ }),
...
describe("Skills 首次目标状态展示", () => {
  it("缺失与已有目录目标都精确展示已分配待同步，并由后续主操作触发同步", async () => {
  ...
  it("首次待同步状态说明后续主操作会自动同步且隐藏手动按钮", async () => {
