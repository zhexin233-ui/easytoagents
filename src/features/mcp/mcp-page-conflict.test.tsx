...
describe("McpPage", () => {
  it("全局状态保留脱敏说明并提供导入入口", async () => {
    ...
    expect(
      within(claudeCard).getByRole("button", { name: "检测并导入已有 MCP" }),
    ).toBeEnabled();
    expect(
      within(claudeCard).queryByRole("button", { name: /预览/ }),
    ).not.toBeInTheDocument();
  });
  it("全局目标状态卡隐藏手动同步按钮", async () => {
    vi.mocked(commands.getAppSettings).mockResolvedValue({
      status: "ok",
  ...
  it("自动同步的预览与 Apply 失败通知按队列堆叠", async () => {
    ...
    vi.mocked(commands.setMcpEnabled).mockResolvedValue({
      status: "ok",
      data: withAffectedSyncScopes({ ...assignedServer, enabled: false }, [
  ...
  it("编辑已分配 MCP 保存后自动同步并 Apply", async () => {
    ...
    vi.mocked(commands.updateMcpServer).mockResolvedValue({
      status: "ok",
      data: withAffectedSyncScopes(assignedServer, [
        globalSyncScope("mcp", "claude"),
      ]),
    });
    renderPage();
  ...
  it("编辑已分配 MCP 遇受管漂移时直接覆盖并 Apply", async () => {
    vi.mocked(commands.getAppSettings).mockResolvedValue({
      status: "ok",
      data: { enabledTools: ["claude", "codex"] },
    });
  ...
  it("未分配的空目标不触发同步，也不展示手动预览", async () => {
    renderPage();
    const statusSection = screen
      .getByRole("heading", { name: "全局目标状态" })
      .closest("section");
