    ...
    database
        ...
        .query_row(
            "SELECT profile.id, profile.name, profile.body, profile.is_active_claude,
                    profile.is_active_codex, profile.is_active_zcode, profile.is_active_cursor,
...
pub fn set_prompt_project_assignment(
    ...
    if let Some(profile_id) = prompt_profile_id {
        ...
        let profile_state: Option<(bool, bool, bool, bool)> = transaction
            .query_row(
                "SELECT is_active_claude, is_active_codex, is_active_zcode, is_active_cursor
        ...
        let globally_enabled = match profile_state {
            Some((claude, codex, zcode, cursor)) => match tool {
                Tool::Claude => claude,
                Tool::Codex => codex,
                Tool::Zcode => zcode,
                Tool::Cursor => cursor,
