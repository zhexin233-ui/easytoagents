-- 放宽 project_native_resources.entry_type CHECK 以允许 'agent_file'：
-- 项目级 Agents 目录中的普通文件按文件名只读登记为项目原生资源。
--
-- 该表没有被任何表以外键引用（只引用 managed_targets 与 snapshots），按
-- database-guidelines 的 12 步表重建执行：本文件内先校验 0021 之后的精确
...
CREATE TEMP TABLE migration_0024_entry_type_anchor (
    matched INTEGER NOT NULL CHECK(matched = 1)
);
INSERT INTO migration_0024_entry_type_anchor(matched)
    SELECT COUNT(*)
    FROM sqlite_master
    WHERE type = 'table'
      AND name = 'project_native_resources'
      AND sql IS NOT NULL
      AND instr(sql, 'entry_type IN (''mcp_entry'', ''directory'', ''symlink'', ''hook_entry'')') > 0;

-- 步骤 3：新表完整复制 0021 的列与约束，仅 entry_type CHECK 增补 'agent_file'。
CREATE TABLE project_native_resources_new (
    ...
    entry_type TEXT NOT NULL CHECK(entry_type IN ('mcp_entry', 'directory', 'symlink', 'hook_entry', 'agent_file')),
...
CREATE TEMP TABLE migration_0024_fk_check (
    violations INTEGER NOT NULL CHECK(violations = 0)
);
INSERT INTO migration_0024_fk_check(violations)
    SELECT COUNT(*) FROM pragma_foreign_key_check;

CREATE TEMP TABLE migration_0024_integrity_check (
    result TEXT NOT NULL CHECK(result = 'ok')
);
INSERT INTO migration_0024_integrity_check(result)
    SELECT * FROM pragma_integrity_check;

DROP TABLE migration_0024_entry_type_anchor;
DROP TABLE migration_0024_row_count;
DROP TABLE migration_0024_fk_check;
DROP TABLE migration_0024_integrity_check;
