...
- Construct every `TargetDescriptor` through `TargetDescriptor::builder(tool,
  ...
  `TargetTrustState::NotRequired`, `PromptOverrideState::NotApplicable`,
  `SymlinkPolicy::Reject`, and no selectors; only set what differs.
- Global `allowed_root` values come from one function:
  `adapters::global_root_for(environment, tool, artifact_kind)`. Discovery fills
  descriptors through `populate_descriptor_allowed_roots`, and snapshot restore
  (`overview::global_allowed_root`) calls the same function after its own
  fail-closed capability gate. Never copy the tool → root table into a service.
- Decode a stored tool string only through `Tool::from_stable_str` (generated by
  `string_enum!`). Inside `src/db/` use `db::column_tool(row, index)`; elsewhere use
  `Tool::from_stable_str(value).ok_or_else(...)` with the caller's existing error
...
- `TargetDescriptor::builder(tool, artifact_kind, scope) -> TargetDescriptorBuilder`
  is the only descriptor constructor; `build()` seeds `allowed_root` from
  `project_root` and `populate_descriptor_allowed_roots(environment, &mut targets)`
  overwrites Global roots with `global_root_for(environment, tool, artifact_kind)`.
...
- Explicit exception to "read commands never write":
  `list_project_native_resources` is a read command that performs a
  transactional reconciliation of `project_native_resources` before it lists.
  This is deliberate (the native list must reflect the current file state, and
  the write is bounded to one `IMMEDIATE` transaction on application-owned
  rows). Do not extend this exception to other read commands. Write commands
  (`register_project`, `rescan_project`) reconcile by design; do not add a
  new read-side reconciliation site without updating this list.
