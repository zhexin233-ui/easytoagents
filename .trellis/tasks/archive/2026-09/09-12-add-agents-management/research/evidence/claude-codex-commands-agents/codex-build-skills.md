...
Use agent skills to extend ChatGPT and Codex with task-specific capabilities. A skill packages instructions, resources, and optional scripts so either product can follow a workflow reliably. Skills build on the [open agent skills standard](https://agentskills.io).

Skills are the authoring format for reusable workflows. Plugins distribute reusable skills and connectors through the universal plugin directory shared by ChatGPT and Codex. Plugins work in Chat and Work across ChatGPT on the web, desktop, and mobile, in Codex in the ChatGPT desktop app, and through Codex CLI. Use skills to design the workflow itself, then package it as a [plugin](https://developers.openai.com/plugins/build/plugins) when you want other people to install it.
...
Skills use **progressive disclosure** to manage context efficiently. ChatGPT and Codex start with each skill's name and description, then load the full `SKILL.md` instructions when they decide to use that skill.
...
1. **Explicit invocation:** Include the skill directly in your prompt. In ChatGPT, type `@` to select a skill. In Codex CLI or the IDE extension, run `/skills` or type `$` to mention a skill.
2. **Implicit invocation:** ChatGPT or Codex can choose a skill when your task matches the skill `description`.
...
If you already know the workflow and it's easier to show than describe, use [Record & Replay](/codex/extend/record-and-replay). The recorder captures the workflow, inspects the steps, and drafts a reusable skill from the demonstration.

If you want to describe the skill instead, use the built-in creator. In ChatGPT Work, invoke it as `@skill-creator`. In Codex, invoke it as:
...
```
--- ---name: skill-name name: skill-namedescription: Explain exactly when this skill should and should not trigger. description: Explain exactly when this skill should and should not trigger. --- --- Skill instructions for ChatGPT or Codex to follow.Skill instructions for ChatGPT or Codex to follow.
```
...
These locations are for authoring and local discovery. When you want to distribute reusable skills beyond a single repo, or optionally bundle them with connectors, use [plugins](https://developers.openai.com/plugins/build/plugins).

## Distribute skills with plugins

Direct skill folders are best for local authoring and repo-scoped workflows. If you want to distribute a reusable skill, bundle two or more skills together, or ship a skill alongside a connector, package them as a [plugin](https://developers.openai.com/plugins/build/plugins).
...
```
$skill-installer linear$skill-installer linear
```
...
```
[[skills.config]][[skills. config]]path = "/path/to/skill/SKILL.md"path = "/path/to/skill/SKILL.md"enabled = falseenabled = false
```
...
```
interface: interface: display_name: "Optional user-facing name"  display_name: "Optional user-facing name" short_description: "Optional user-facing description"  short_description: "Optional user-facing description" icon_small: "./assets/small-logo.svg"  icon_small: "./assets/small-logo.svg" icon_large: "./assets/large-logo.png"  icon_large: "./assets/large-logo.png" brand_color: "#3B82F6"  brand_color: "#3B82F6" default_prompt: "Optional surrounding prompt to use the skill with"  default_prompt: "Optional surrounding prompt to use the skill with" policy: policy: allow_implicit_invocation: false  allow_implicit_invocation: false dependencies: dependencies: tools:  tools: - type: "mcp" - type: "mcp" value: "openaiDeveloperDocs"  value: "openaiDeveloperDocs" description: "OpenAI Docs MCP server"  description: "OpenAI Docs MCP server" transport: "streamable_http"  transport: "streamable_http" url: "https://developers.openai.com/mcp"  url: "https://developers.openai.com/mcp"
```
