...
Add reusable workflows, expertise, and connected tools to ChatGPT and Codex
...
* A **plugin** is an installable bundle that can include skills and Model Context Protocol (MCP) servers. MCP servers expose tools and can optionally include custom ChatGPT UI. Plugins can also include [lifecycle hooks](/codex/hooks) for the Codex runtime, including ChatGPT Work and Codex.
...
Plugins make reusable capabilities easier to install and share. A plugin can combine skills with MCP servers for tools and context from services such as GitHub, Google Drive, or Slack.
...
You can also demonstrate a workflow with [Record & Replay](/codex/extend/record-and-replay), which turns the recording into a reusable skill. To package and distribute your own bundle, see [Build plugins](https://developers.openai.com/plugins/build/plugins).

If your plugin needs to connect to a service or expose MCP tools, see [Build an MCP server](https://developers.openai.com/plugins/build/mcp-server). When your plugin is ready for public review, see [Submit plugins](https://developers.openai.com/plugins/deploy/submission).

For more examples of reusable workflows, see [Using skills in OpenAI Academy](https://openai.com/academy/skills/).
