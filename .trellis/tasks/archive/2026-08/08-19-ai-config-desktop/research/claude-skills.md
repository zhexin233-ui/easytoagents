...
```
my-skill/my-skill/├── SKILL.md # Main instructions (required)├── SKILL.md # Main instructions (required)├── template.md # Template for Claude to fill in├── template.md # Template for Claude to fill in├── examples/├── examples/│ └── sample.md # Example output showing expected format│ └── sample.md # Example output showing expected format└── scripts/└── scripts/ └── validate.sh # Script Claude can execute └── validate.sh # Script Claude can execute 
```
...
```
--- ---name: migrate-component name: migrate-componentdescription: Migrate a component from one framework to another description: Migrate a component from one framework to another --- --- Migrate the $0 component from $1 to $2.Migrate the $0 component from $1 to $2.Preserve all existing behavior and tests.Preserve all existing behavior and tests.
```
...
```
## Environment ## Environment```!```!node --versionnode --versionnpm --versionnpm --versiongit status --shortgit status --short ``` ```
```
